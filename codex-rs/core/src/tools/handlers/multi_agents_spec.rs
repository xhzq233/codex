...
fn plaintext_inter_agent_message_schema(description: &str) -> JsonSchema {
    // Plaintext collaboration tools use a non-reserved namespace so relay providers can consume
    // the portable Responses schema without the encrypted field extension.
